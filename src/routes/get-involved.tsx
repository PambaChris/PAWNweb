    ...
    <div className="min-h-screen bg-sand text-ink">
      ...
      <header className="px-6 md:px-8 pt-16 pb-12 max-w-6xl mx-auto">
        ...
        <h1 className="text-5xl md:text-7xl font-serif leading-[0.95] max-w-3xl text-balance">
          Find your <span className="italic text-clay">place</span> in this movement.
        </h1>
      ...
      <section className="px-6 md:px-8 max-w-6xl mx-auto pb-24">
        <div className="grid md:grid-cols-2 gap-8">
          {paths.map((p) => (
            ...
            >
              ...
              <Link
                to="/contact"
                className="self-start bg-ink text-sand px-6 py-3 rounded-full text-xs uppercase tracking-widest font-medium hover:bg-clay transition-colors"
              >
                {p.cta}
              </Link>
            </div>
        ...
        <div className="mt-16 bg-clay text-sand rounded-3xl p-12 md:p-16 text-center">
          <h2 className="text-3xl md:text-5xl font-serif mb-6 max-w-2xl mx-auto text-balance">
            Stay close to the work.
          </h2>
          <p className="text-sand/85 max-w-xl mx-auto mb-8 leading-relaxed">
            Sign up for our newsletter — event invitations, calls to action,
            and dispatches from chapters worldwide.
          </p>
          <Link
            to="/contact"
            className="inline-flex bg-sand text-clay px-10 py-4 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-gold hover:text-ink transition-all"
          >
            Subscribe
          </Link>
        </div>
