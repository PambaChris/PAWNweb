export const upcomingEvents = [
  {
    title: 'Annual Leadership Summit',
    date: 'October 25-27, 2024',
    location: 'Online',
    description: 'A weekend of inspiring talks, workshops, and networking opportunities with leaders from across the diaspora.',
    image: '/news-annual-leadership-summit.jpg',
    slug: 'annual-leadership-summit'
  },
  {
    title: 'Tech & Innovation Workshop',
    date: 'November 15, 2024',
    location: 'Lagos, Nigeria & Online',
    description: 'A hands-on workshop for women looking to break into or advance in the tech industry.',
    image: '/news-mentorship-program.jpg',
    slug: 'tech-innovation-workshop'
  },
  {
    title: 'Cultural Heritage Gala',
    date: 'December 12, 2024',
    location: 'Accra, Ghana',
    description: 'A celebration of African culture, art, and music. A night to remember!',
    image: '/news-financial-independence.jpg',
    slug: 'cultural-heritage-gala'
  },
];

export const pastEvents = [
  {
    title: '16 Days of Activism Against Gender-Based Violence and Political Spa',
    date: 'December 9, 2025',
    slug: '16-days-of-activism-against-gender-based-violence-and-political-spa',
    image: '/kuillust.jpeg',
    description: 'The Pan African Women\'s Network partnered with the Kenyatta University center for Gender and Empowerment to hold an event on the 16 Days of Activism against gender based violence and political Spa. Activities included panel discussions,wellness: Music,dancing and art,Political Spa and Plenary on GBV manifestations and especially Technologically Facilitated GBV. The session was enlightening. The event started at 2pm at Kenyatta University.',
    schedule: [
      { time: '2:00pm - 2:10pm', activity: 'Opening Remarks' },
      { time: '2:10pm - 2:40pm', activity: 'War on Sudan Documentary' },
      { time: '2:40pm - 3:00pm', activity: 'Reflections' },
      { time: '3:00pm - 3:30pm', activity: 'Panel' },
      { time: '3:30pm - 4:00pm', activity: 'Panel 2' },
      { time: '4:00pm - 4:15pm', activity: 'Keynote speech' },
      { time: '4:15pm - 4:30pm', activity: 'Plenary' },
      { time: '4:30pm - 4:45pm', activity: 'Breakout Session' },
      { time: '4:45pm - 5:15pm', activity: 'Artistic Session' },
      { time: '5:15pm - 5:45pm', activity: 'Mini concert' },
      { time: '5:45pm - 6:00pm', activity: 'Closing Remarks' },
      { time: '6:00pm', activity: 'Refreshments/Departure' },
    ]
  },
  {
    title: 'Financial Literacy Seminar',
    date: 'August 2025',
    slug: 'financial-literacy-seminar',
    image: '/news-financial-independence.jpg'
  },
  {
    title: 'Inaugural Conference',
    date: 'May 2025',
    slug: 'inaugural-conference',
    image: '/inauguralconference.jpg'
  },
  {
    title: 'Solidarity With Palestine',
    date: 'February 2025',
    slug: 'solidarity-with-palestine',
    image: '/WhatsApp Image 2025-12-03 at 4.19.43 PM.jpeg'
  },
  {
    title: 'Social Justice: Githurai Kimoja Festival',
    date: 'March 2025',
    slug: 'social-justice-githurai-kimoja-festival',
    image: '/KimojaMain.jpeg'
  },
  {
    title: 'Solidarity With Venezuela',
    date: 'April 2025',
    slug: 'solidarity-with-venezuela',
    image: '/internacionalvenezuela.jpeg'
  },
  {
    title: 'Grassroots Feminist Women Education',
    date: 'June 2025',
    slug: 'grassroots-feminist-women-education',
    image: '/feministgrassroosts.jpeg'
  },
  {
    title: 'Solidarity With The Families Of Victims Of The Gen-Z Protests',
    date: 'July 2025',
    slug: 'solidarity-with-the-families-of-victims-of-the-gen-z-protests',
    image: '/genz fahm.jpeg'
  },
  {
    title: 'Study Circle',
    date: 'September 2025',
    slug: 'study-circle',
    image: '/studycircle.jpeg'
  },
];
